...
"""Commands for PIXIS 1024B camera. Uses variables in command.py and port_utils.py."""
...
def pixis_take_bias_frames(n_bias):
    ...
    bias_exposure_time = 0.0 #exposure time set to 0 for bias frames 
    ...
    while count<=n_bias: 
        ...
        # 5 Always high
        # 6 aquiring
        # 7 shifting under mask
        # 8 exposing
        # 10 reading out
        # 11 waiting for trigger
...
def experimentfilesettings(pix_filenames_basename,pix_folder_name):
    ...
    pixis_set_value(ExperimentSettings.OnlineExportFormat,1.0) #  1 for FITS format
    ...
    pixis_set_value(CameraSettings.ShutterTimingMode,AddIns.ShutterTimingMode(1)) # 2 open the shutter when exposure is taken, 3 for shutter remains closed when exposure is taken.  When Trigger ouput set to "Shutter"
    ...
    pixis_set_value(CameraSettings.AdcSpeed,2.0) #set to 2 MHz, other option is 0.1 MHz
    pixis_set_value(CameraSettings.AdcGain,1.0) #set Adc Gain to 1, other values are 2, 3 
