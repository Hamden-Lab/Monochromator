...
def whereishome(): 
    ...
    """
    home_wl: float = np.round(420.64,2) 
    #howe_wl = np.round(631.26,2)
    return home_wl
...
def home(MCport):
    ...
    try:
        ...
        ser.close()
        ser.open()
        ser.write(b'-108000 \r'); #turns motor for 3 rev., subtracts 12nm. Used to prevent power switch failure in scan controller seen in testing
        ser.read_until(size=None)
        ser.close()
        ser.open()
...
def go_to_fromhome(MCport,wl):
    ...
    try:
        ...
        uplim = 900.0 #nm
        lowlim = 100.0 #nm
        home_wl: float = np.round(420.64,2) #nm
        grating_sf=1.49772 #1.5 for 1800 gpm grating and 1 for 1200 gpm grating
        rev = grating_sf*9000 #steps #steps
...
#         home_wl: float = np.round(420.64,2) #nm
#         grating_sf=1.49772 #1.5 for 1800 gpm grating and 1 for 1200 gpm grating 
#         rev = grating_sf*9000 #steps.0 #steps
#         #equation to find difference between home position and new wavelength desired
...
#         home_wl: float = np.round(420.64,2) #nm
#         grating_sf=1.49772 #1.5 for 1800 gpm grating and 1 for 1200 gpm grating
#         rev = grating_sf*9000 #steps #steps = 1 nm 
#         currentwave=home_wl
#         print("here")
#         # print(scanarray)
